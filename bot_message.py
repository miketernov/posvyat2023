...
from bot_get_fuctions import get_student_info, check_if_in, check_if_in_org, get_org_schedule, check_if_in_tags, get_my_schedule_now, get_my_schedule
...
kb1 = [
    [types.KeyboardButton(text="Посмотреть мою точку на данный момент")],
    [types.KeyboardButton(text="Посмотреть мое дальнейшее расписание")],
...
async def send_on(message: types.Message):
    ...
    if fl is True:
        ...
        await message.answer("К сожалению, вы не являетесь организатором, вход воспрещен!")


@dp.message_handler(lambda message: message.text == "Посмотреть мою точку на данный момент")
async def send_on(message: types.Message):
    username = message.from_user.username
    result = get_my_schedule_now(username)
    keyboard = types.ReplyKeyboardMarkup(keyboard=kb1, resize_keyboard=True)
    await message.answer(result, reply_markup=keyboard)


@dp.message_handler(lambda message: message.text == "Посмотреть мое дальнейшее расписание")
async def send_on(message: types.Message):
    username = message.from_user.username
...
async def process_fio_invalid(message: types.Message):
    ...
    return await message.reply(
        "Данные введены неверно, либо данного человека нет с нами! Попробуйте еще раз или вернитесь в меню!",
...
async def process_fio_invalid(message: types.Message):
    keyboard = types.ReplyKeyboardMarkup(keyboard=kb3, resize_keyboard=True)
    return await message.reply(
        "Данные введены неверно, либо данного человека нет в списке! Попробуйте еще раз или вернитесь в меню!",
        reply_markup=keyboard)
...
scheduler.add_job(send_on_schedule, trigger='cron', hour='*', minute=', '.join(map(str, send_mins)), start_date="2023-07-29 7:59:55.000000", end_date="2023-07-30 16:00:30.000000")
scheduler.start()
